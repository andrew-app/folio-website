...
# <WasmIcon/> for AWS Lambda@Edge functions
...
### **The Motivation**
I think it is important to first outline the problem we are trying to solve with Lambda@Edge and the accompanying AWS infrastructure.
...
One feature that is missing from actions which was present in rules was the existence of a datastore. In rules we could store data in a global object and access it in subsequent rules.
<br/>
...
which allows to run code as close to the user and manipulate the request and response without having to worry about servers and the other accompanying infrastructure
...
<br/>
With the use of Lambda@Edge, CLoudfront with its caching ability and S3 whcih will hold our data in the form of JSON files we can be confident that when auth0 actions has to access the datastore it will be pretty quick to not adversely affect the user experience when logging in. 
### **About Lambda@Edge**
AWS Lambda@Edge functions are used to run code in response to CloudFront events. These events include viewer requests, origin requests, viewer responses, and origin responses. The functions are executed in AWS locations closer to the end user, which can reduce latency and improve performance.
